...
- Run setup diagnostics with `kg doctor`.
- Run graph integrity diagnostics with `kg check`.
...
| `kg doctor` | Run local setup diagnostics for config, deps, and connectivity |
| `kg check` | Run integrity diagnostics and fail on stale graph artifacts |
...
1. Execute remaining v2 phase 0 and phase 1 ingest correctness items.
2. Expand investigation workflow in web UI.
3. Add richer trust/evidence exploration in the web UI.

### Next

- Expand investigation workflow in web UI.
- Add richer trust/evidence exploration in the web UI.
- Add deeper integrity and repair workflows on top of `kg check`.
